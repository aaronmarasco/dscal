...
from dscal.models import Kind, is_deadline
from dscal.parser import parse_events


def test_parses_all_labeled_events(dsc80_html: str) -> None:
    events = parse_events(dsc80_html, course="DSC 80")
    assert len(events) == 5  # 2 LEC, LAB 1, PROJ 1, EXAM
...
def test_event_fields(dsc80_html: str) -> None:
    ...
    assert proj.summary == "DSC 80: PROJ 1"
...
def test_deadlines_only(dsc80_html: str) -> None:
    events = parse_events(dsc80_html, course="DSC 80")
    deadlines = [e for e in events if is_deadline(e.label)]
